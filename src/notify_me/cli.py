...
def run_command_with_notification(command: List[str], message: Optional[str] = None) -> int:
    ...
    # Send notification
    notification_message = notifier.format_command_completion_message(
        command_str, exit_code, execution_time, message
    )
...
Examples:
  ...
  notify-me --exec python train.py        # Run command with notification wrapper
    ...
    parser.add_argument(
        '-m', '--message',
        type=str,
        help='Custom message to send (default: "Task complete")'
    )
    
    parser.add_argument(
        '--exec',
        action='store_true',
        help='Execute command with notification wrapper (old behavior)'
    )
    
    parser.add_argument(
        'cmd',
        nargs='*',
        help='Command to execute when using --exec, or special commands (setup/test)'
    )
